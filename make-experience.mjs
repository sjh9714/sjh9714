...
const ROWS = [
  ...
  {
    year: "2026",
    title: "FINMATE — HANA FINANCIAL GROUP × FSS · YOUTH FINANCE TALENT PROGRAM",
    desc: "청년 금융 온보딩 서비스 — 앱과 Spring Boot 백엔드 단독 구현, 화면의 모든 수치는 거래 원장에서 파생",
  },
  {
    year: "2026",
    title: "MY ETA — HANA × SK TELECOM TECH4GOOD HACKATHON · 24H",
    desc: "표준 보행속도가 틀리는 사람들을 위한 길찾기 — 저장소 커밋의 91%",
  },
  {
    year: "2024",
...
if (process.argv.includes("--subset")) {
  ...
  for (const r of ROWS) for (const ch of r.year + r.title + r.desc) chars.add(ch);
  chars.add("·");
  execFileSync("uvx", [
...
const THEMES = {
  light: { fg: "#0a0b0d", muted: "#5a5f6e", line: "#0a0b0d", sep: "#dcdee8", accent: "#0016ec" },
  dark: { fg: "#f0f1fa", muted: "#9aa1b4", line: "rgba(240,241,250,0.7)", sep: "rgba(240,241,250,0.14)", accent: "#c1ff00" },
};
...
function svg(t) {
  const rows = ROWS.map((r, i) => {
    ...
    <text x="128" y="${y + 34}" class="title">${r.title}</text>
...
console.log(`experience 생성 — ${ROWS.length}행`);
